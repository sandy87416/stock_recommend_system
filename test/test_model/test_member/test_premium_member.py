...
class TestPremiumMember(TestCase):
    ...
    def test_read_recommended_stock(self):
        ...
        self.assertTrue(
            {'stock_id': str(8080), 'stock_name': '永利聯合', 'rsi_6': str(100.0),
             'odds': str(0.93)} in recommended_stock_list)
        self.assertTrue({'stock_id': str(8477), 'stock_name': '創業家', 'rsi_6': str(75.61),
                         'odds': str(0.89)} in recommended_stock_list)
        self.assertEqual(len(recommended_stock_list), 2)

        recommended_stock_list = self.premium_member.read_recommended_stock(5, 0.8)
        self.assertTrue({'stock_id': str(4712), 'stock_name': '南璋', 'rsi_6': str(60.71),
                         'odds': str(0.93)} in recommended_stock_list)
        self.assertTrue(
            {'stock_id': str(9802), 'stock_name': '鈺齊-KY', 'rsi_6': str(92.0),
             'odds': str(0.93)} in recommended_stock_list)
        ...
        self.assertTrue(
            {'stock_id': str(8080), 'stock_name': '永利聯合', 'rsi_6': str(100.0),
             'odds': str(0.93)} in recommended_stock_list)
        self.assertTrue(
            {'stock_id': str(8477), 'stock_name': '創業家', 'rsi_6': str(75.61),
             'odds': str(0.89)} in recommended_stock_list)
    ...
    def test_read_stock_odds(self):
        ...
        self.assertEqual(stock_odds_list[0],
                         {'stock_id': str(2330), 'stock_name': '台積電', 'days': str(2), 'odds': str(0.69)})
        self.assertEqual(stock_odds_list[1],
                         {'stock_id': str(2330), 'stock_name': '台積電', 'days': str(3), 'odds': str(0.6)})
